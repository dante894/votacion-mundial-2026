...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if($action === 'update_score') {
        ...
        // Si es una llamada AJAX (fetch), responder con JSON y salir
...
<aside>
  ...
  <button class="aside-link" id="sync-btn" onclick="syncFifa()"><span class="icon">🔄</span>Sincronizar FIFA</button>
</aside>
...
<div class="tab" id="tab-matches">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
    <h2 style="font-size:20px;font-weight:700">Partidos (<?=count($matches)?>)</h2>
    <button class="btn btn-primary" id="sync-top-btn" onclick="syncFifa()" style="display:flex;align-items:center;gap:6px;font-size:13px">
      <span id="sync-icon">🔄</span> Sincronizar con FIFA
    </button>
  </div>
  <div id="sync-log" style="display:none;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:14px;margin-bottom:16px;font-size:12px;font-family:monospace;max-height:180px;overflow-y:auto;color:var(--muted)"></div>
    ...
    <table>
      ...
      ?>
        <tr>
          ...
          <td>
            <div style="display:flex;gap:6px;align-items:center" id="score-row-<?=$m['id']?>">
              <!-- Score form AJAX (no recarga la página) -->
              <div style="display:flex;gap:4px;align-items:center">
                <input type="number" id="hs-<?=$m['id']?>" value="<?=$m['home_score']??''?>" placeholder="0"
                       style="width:44px;padding:4px 6px;font-size:12px;background:rgba(255,255,255,.04);border:1px solid var(--border);border-radius:6px;color:var(--text);text-align:center">
                <span style="color:var(--muted);font-size:11px">-</span>
                <input type="number" id="as-<?=$m['id']?>" value="<?=$m['away_score']??''?>" placeholder="0"
                       style="width:44px;padding:4px 6px;font-size:12px;background:rgba(255,255,255,.04);border:1px solid var(--border);border-radius:6px;color:var(--text);text-align:center">
                <select id="st-<?=$m['id']?>" style="padding:4px 6px;font-size:12px;background:var(--card);border:1px solid var(--border);border-radius:6px;color:var(--text)">
                ...
                <button class="btn btn-primary btn-sm" id="sb-<?=$m['id']?>"
                        onclick="saveScore(<?=$m['id']?>)">✓</button>
              </div>
              <!-- Delete -->
...
// ── Guardar resultado SIN recargar (mantiene posición del scroll) ─────────
...
async function saveScore(matchId) {
  ...
  try {
    ...
    fd.append('home_score', hs);
    fd.append('away_score', as_);
    fd.append('status',     st);
    fd.append('csrf',       CSRF);
    ...
    // Feedback visual
    const row = document.getElementById('score-row-'+matchId);
...
// ── Sincronizar con FIFA ─────────────────────────────────────────────────
async function syncFifa() {
  const btns    = [document.getElementById('sync-btn'), document.getElementById('sync-top-btn')];
  ...
  logBox.innerHTML = '<span style="color:var(--gold)">Conectando con FIFA.com…</span>';

  try {
    const fd = new FormData();
    fd.append('csrf', CSRF);

    const res  = await fetch('sync_fifa.php', { method: 'POST', body: fd,
      headers: { 'X-CSRF-TOKEN': CSRF } });
    const data = await res.json();

    const lines = (data.logs || []).map(l => {
      const color = l.includes('✓') ? '#22c55e' : l.includes('✗') ? '#ef4444' : l.includes('⚠') ? '#f0b429' : '#5a6a7a';
      return `<span style="color:${color}">${l.replace(/</g,'&lt;')}</span>`;
    });

    logBox.innerHTML = lines.join('<br>') || 'Sin datos.';
    ...
    logBox.innerHTML = `<span style="color:var(--red)">Error: ${e.message}</span>`;
    if(icon) icon.textContent = '❌';
  } finally {
